...
#### Решение
Рассмотрим пакет, в котором находится интересующий нас бит. После попадания в пакет бит ждал, когда все биты после него в этом пакете будут закодированы, а при достижении хоста Б ждал, пока все биты перед ним будут и он сам будут декодированы, то есть он ждал $56*8/(128 * 1024)$ секунд. Помимо этого  $56*8/(1024 * 1024) + 0.005$ секунд было потрачено на передачу по каналу (скорость передачи + задержка распространения). Значит интересующее нас время равно $0.088452$
...
#### Решение
$a = N/d = 10 / (0.01 + 0.01) = 500$ пакетов в секунду.
...
#### Решение
1. После отправки первого пакета в сеть пройдёт $L/R_s$ секунд, после этого начнётся отправка второго пакета, а поскольку первая линия медленнее второй, значит второй пакет отправится по ней раньше, чем первый дойдёт до маршрутизатора, а значит и общая разница в прибытии на хост составит $L/R_s$.
2. Через $L/R_s + d_{распространения} + L/R_c$ первый пакет выйдет из маршрутизатора, в то время как второй пакет доберётся до маршрутизатора за время равное $2 * L/R_s + d_{распространения}$. Поскольку первая линяя быстрее второй, второй пакет будет ждать, пока первый выйдет из маршрутизатора (то есть ответ на первую часть вопроса - да). Для того, чтобы избавиться от очереди нужно, чтобы T было хотя бы равно разнице между временем, необходимым первому пакету для выхода из маршрутизатора и временем, необходимым второму пакету для достижения маршрутизатора. То есть минимальное T равно $L/R_c-L/R_s$.
