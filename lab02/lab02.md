...
#### Демонстрация работы
Скриншоты приложены к описаниям запросов.
...
#### Решение
Заметим, что на первом соединении каждый пакет должен дождатьсья отправки всех пакетов, которые стоят раньше него, а на каждом последующем таких ожиданий не будет, так как все соединения имеют одинаковую скорость, таким образом задержка последнего пакета (а следовательно и общая задержка) составит $(N + P - 1) * L / R$ 
...
#### Решение
Пропускная способность данного соединения - это min($R_1$, $R_2$, $R_3$) = 200 Кбит/с.
Значит для передачи файла размером $5$ мегабайт потребуется примерно $5 * 8 * 10^6 / (200 * 10^3) = 200$ секунд.
...
#### Решение
Посчитаем вероятность $P$ - вероятность того, что одновременных пользователей меньше $12$. Пусть $p(x) = C^x_{60} * (0.2) ^ x * (0.8) ^ {(60 - x)}$ - вероятность того, что ровно x пользователей одновременно передают данные. Тогда $P = p(0) + p(1) + ... + p(11)$, а искомая величина равна $1 - P$. Ответ: 0.55
...
#### Решение
Рассмотрим, когда последний пакет дойдёт до хоста Б: $(80 + S) * X / (R * S) + (80 + S) * 2 / R$. Первое слагаемое - это время, которое потребуется последнему пакету, чтобы добраться до первого коммутатора (он должен дождаться отправки всех остальных пакетов), второе слагаемое - это время, за которое последний пакет дойдёт до хоста Б. Чтобы найти оптимальны $S$ возьмём производную этого выражения, получится $2 * (S^2 - 40 * X) / (S^2 * R)$, сначала функция убывает, потом возрастает, значит оптимальный $S = min(X, 2 * \sqrt{10 * X})$ 
