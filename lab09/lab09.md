...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Он является протоколом сетевого уровня, а не транспортного
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Type 8, Code 0
   - Checksum, Identifier, Sequence Number и Data
   - 2 байта на каждое поле
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Больше информации в TTL, Data увеличилась в 2 раза
   Ping:
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Поле unused, данные пакета IP , данные пакета ICMP
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - У них одинаковый TTL и он больше, чем у предыдущих и в них нет информации о прошлых пакетах.
   - Это пакеты, которым хватило TTL чтобы дойти до хоста и поэтому в них не нужно хранить лишнюю информацию о старых пакетах.
...
#### Решение
1. За период было отправлено $W/2 + (W/2 + 1) + ... + W = ((3 * W / 2) * (W / 2 + 1))/2=3* W^2/8 + 3 * W/4$ сегментов и 1 сегмент был потерян, значит вероятность потери $L=1/(3 * W^2/8 + 3 * W/4)$.
2. Из первого пункта $W\approx\sqrt{8/(3 * L)}$, а также мы знаем, что средняя скорость - это $A = 3 * W * MSS/(4 * RTT)$, значит $\sqrt{3/2} * MSS / (RTT * \sqrt{L})$.
