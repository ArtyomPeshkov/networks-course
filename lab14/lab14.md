...
2. Какому устройству соответствует значение Ethernet-адреса?
   ...
   - сетевому адаптеру на моём компьютере
...
#### Решение
1. Вероятность успешной отправки в квант равна $N * p * (1-p)^{N-1}$ (захотел отправить ровно один, не важно какой). x равно количеству шагов до первой успешной отправки, так как именно в этот момент мы вызодим из «непродуктивного состояния». $x = (1 - N * p * (1-p)^{N-1}) / (N * p * (1-p)^{N-1})$
Значит эффективность этого протокола равна $k/(k+x)$ для найденного нами x.

2. Нам нужно минимизировать x при заданном N, что эквиваленто минимизации $N * p * (1-p)^{N-1}$. Из 13 задания мы вычислили, что это выражение минимизируется при $p=1/N$.

3. Опять же из 13 задания мы знаем, что при таком p и N -> беск значение x стремится к $e^{-1}$, а значит эффективность протокола стремится к $k/(k+e^{-1})$

4. Увеличение размера пакета равносильно увеличению значения k. При фиксированных значениях N и p значение x тоже фиксированное, а значит эффективность ($k/(k + x)$) стремится к 1. ЧТД.
...
#### Решение
1. Задержка равна $(L+5)/(128*1024/8) = (L+5)/16384$ с.
2. Для Ethernet-пакета задержка равна $(1500+5)/16384 = 91.9$ мс, а для ATM-пакета задержка равна $(50+5)/16384 = 3.4$ мс.
3. Скорость передачи равна $A = 622 * 1024 * 1024 / 8 = 81526784$ б/c, тогда ответ равен $1505/81526784 = 0.018$ мс и $55/81526784 = 0.00067$ мс.
4. Меньше задержка пакетирования, а значит будет меньше эфирного эхо. Также повторные отправки пакетов будут происходит быстрее и реже. Более того любой анализ корректности и обработка в случае ошибок будет происходить быстрее.
