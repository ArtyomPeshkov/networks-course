...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - 128.119.245.12
   - 80 и для приёма и для отправки
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence Number (raw): 135744662, Sequence Number: 0 (relative sequence number)
   - По выставленному флагу SYN в заголовке
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Sequence Number (raw): 2926146775, Sequence Number: 0    (relative sequence number)
   - 1 (Acknowledgment)
   - Это увеличенное на 1 Sequence Number пакета, пришедшего от пользователя.
   - По выставленному флагу SYN и ACK в заголовке
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Sequence Number (raw): 135744663, Sequence Number: 1    (relative sequence number)
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - 1 610 2070 3530 4990 6450
   - Первый: 7.022402, Остальные: 7.022536
   - Он был получен в 7.143628
   - RTT: 0.121c
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - (7910 - 610)/0.121 = 60330.5785 байт/с
   - Возьмём разницу между порядковыми номерами сегментов со 2 по 6 и поделим на время, которое прошло между отправкой запроса и получением ответа
...
#### Решение
Из формулы с лекции мы хотим, чтобы $(n * 1500 * 8 / 10^9)/(0.03 + 1500 * 8 / 10^9) > 0.98$. Значит n должно быть больше
$2450$.